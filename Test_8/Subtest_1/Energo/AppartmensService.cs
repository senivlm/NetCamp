...
namespace Energo
{
    internal class AppartmensService
    {
        ...
        public AppartmensService(List<Appartment> appartments, int quarter)
        {
        ...
        public static AppartmensService operator +(AppartmensService a, AppartmensService b)
        {
            List<Appartment> appartmentsRes = b.appartments.ToList();
            foreach (Appartment appartment in a.appartments)
            {
                if (!b.appartments.Any(b => b.Number == appartment.Number && b.FIO.Equals(appartment.FIO)))
                    appartmentsRes.Add(appartment);
            }
            return new AppartmensService(appartmentsRes, b.quarter);
        }
        public static AppartmensService operator -(AppartmensService a, AppartmensService b)
        {
            List<Appartment> appartmentsRes = a.appartments.ToList();
            foreach (Appartment appartment in a.appartments)
            {
                if (b.appartments.Any(b => b.Number == appartment.Number && b.FIO.Equals(appartment.FIO)))
                    appartmentsRes.Remove(appartment);
            }
            return new AppartmensService(appartmentsRes, a.quarter);
        }
